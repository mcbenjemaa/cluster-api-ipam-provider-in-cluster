...
var _ = Describe("IPAddressClaimReconciler", func() {
	...
	Context("When a new IPAddressClaim is created", func() {
		...
		When("the referenced namespaced pool exists", func() {
			...
			It("should allocate an Address from the Pool", func() {
				...
				Eventually(func(g Gomega) {
					readyCondition := claimReadyCondition(g, "test", namespace)
					g.Expect(readyCondition).NotTo(BeNil())
					g.Expect(readyCondition.Status).To(Equal(metav1.ConditionTrue))
					g.Expect(readyCondition.Reason).To(Equal(ipamutil.IPAddressClaimReadyAddressAllocatedReason))
		...
		When("the referenced namespaced pool does not exists", func() {
			...
			It("should not allocate an Address from the Pool", func() {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(readyCondition.Reason).To(Equal(ipamv1.IPAddressClaimReadyPoolNotReadyReason))
				}).WithTimeout(5 * time.Second).WithPolling(100 * time.Millisecond).Should(Succeed())
			})
		})
		When("the referenced namespaced pool has addresses that overlap with reserved addresses", func() {
			...
			It("does no allocate the reserved addresses", func() {
				...
				Eventually(func(g Gomega) {
					readyCondition := claimReadyCondition(g, "test2", namespace)
					g.Expect(readyCondition).NotTo(BeNil())
					g.Expect(readyCondition.Status).To(Equal(metav1.ConditionFalse))
					g.Expect(readyCondition.Reason).To(Equal(ipamv1.IPAddressClaimReadyPoolExhaustedReason))
				}).WithTimeout(5 * time.Second).WithPolling(100 * time.Millisecond).Should(Succeed())
			})
		})
	...
	Context("When a pool has an address reuse grace period", func() {
		When("a claim is deleted", func() {
			const poolName = "grace-period-pool"
			...
			BeforeEach(func() {
				pool := v1alpha2.InClusterIPPool{
					...
					Spec: v1alpha2.InClusterIPPoolSpec{
						...
						Prefix:                         24,
						Gateway:                        "10.0.0.1",
						AddressReuseGracePeriodSeconds: ptr.To(int32(2)),
					},
				}
				Expect(k8sClient.Create(context.Background(), &pool)).To(Succeed())
				Eventually(Get(&pool)).Should(Succeed())
			...
			AfterEach(func() {
				deleteClaimUnchecked("grace-test", namespace)
				deleteClaimUnchecked("grace-test2", namespace)
				deleteNamespacedPool(poolName, namespace)
			})

			It("should delay IPAddress deletion until grace period elapses", func() {
				claim := newClaim("grace-test", namespace, "InClusterIPPool", poolName)
				Expect(k8sClient.Create(context.Background(), &claim)).To(Succeed())

				// Wait for address to be allocated
				...
				// IPAddress should still exist during grace period
				address := ipamv1.IPAddress{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "grace-test",
						Namespace: namespace,
		...
		When("the pool is deleted before the claim (cluster teardown)", func() {
			const poolName = "grace-teardown-pool"

			It("should skip grace period and clean up immediately", func() {
				pool := v1alpha2.InClusterIPPool{
					ObjectMeta: metav1.ObjectMeta{
						Name:      poolName,
						Namespace: namespace,
					...
					Spec: v1alpha2.InClusterIPPoolSpec{
						...
						Prefix:                         24,
						Gateway:                        "10.0.0.1",
						AddressReuseGracePeriodSeconds: ptr.To(int32(300)),
					},
				}
				Expect(k8sClient.Create(context.Background(), &pool)).To(Succeed())
				Eventually(Get(&pool)).Should(Succeed())

				claim := newClaim("teardown-test", namespace, "InClusterIPPool", poolName)
				Expect(k8sClient.Create(context.Background(), &claim)).To(Succeed())

				// Wait for address to be allocated
				Eventually(findAddress("teardown-test", namespace)).
					WithTimeout(5 * time.Second).WithPolling(100 * time.Millisecond).Should(
...
func claimReadyCondition(g Gomega, name, namespace string) *metav1.Condition {
	claim := ipamv1.IPAddressClaim{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
	...
	g.Expect(k8sClient.Get(context.Background(), client.ObjectKeyFromObject(&claim), &claim)).To(Succeed())
	return conditions.Get(&claim, ipamv1.IPAddressClaimReadyCondition)
}
